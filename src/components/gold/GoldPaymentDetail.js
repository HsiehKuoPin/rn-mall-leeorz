...
class PaymentDetailView extends Component {
    ...
    _loadPayOrder(pass) {
        let requestObj = {
            ...
            'goodsRecordInfoVo': {
                'goodsNumber': itemData.goodsNumber,
                'activityId': itemData.activityId,
                'beneficiaryName': itemData.beneficiaryName,
                'beneficiaryIdentityCard': itemData.beneficiaryIdentityCard,
                'beneficiaryPhone': itemData.beneficiaryPhone,
                'balance': itemData.balance,
                'jadeIntegral': itemData.jadeIntegral,
                'payPassword': pass
            }

        };
        let url = 'order/companyOrderGoods/addCompanyOrderGoods';
        post(url, requestObj, true)
            .then((responseData) => {
                if (isSuccess(responseData)) {
                    showToastShort(responseData.message);
                    this.props.dispatch(gotoAndClose('GoldOrderIndex', ['Main'],{orderType:'ORDER_RECORD'}));
                }
                else {
                    showToastShort(responseData.message);
                }
                ...
                <View style={{flexDirection: 'row'}}>
                    ...
                    <View style={{flexDirection: 'column', marginLeft: 30,}}>
                        <Text style={{
                            ...
                            {formatMoney(itemData.balance, false)}
                        </Text>
                        <Text style={{
                            textAlign: 'left',
                            marginTop: 10,
                            ...
                            {formatMoney(itemData.jadeIntegral, false)}
                        </Text>
                    </View>
                </View>

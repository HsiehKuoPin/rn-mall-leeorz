...
import {goBack, gotoAndClose} from "../../../../reducers/RouterReducer";
import {ORDER_COMPLETED} from "../../../../constraint/OrderType";
import {isTrue} from "../../../../common/AppUtil";

var orderId = null;

class CartFinalPaymentDetailView extends Component {
    ...
    _loadPaymentDetail() {
        ...
        post('order/car/balancePayment', requestObj)
            .then((responseData) => {
    ...
    _loadPayOrder(obj) {
        ...
        post('order/car/payBalancePayment', requestObj,true)
            .then((responseData) => {
                if (isSuccess(responseData)) {
                    showToastShort('支付成功');
    ...
    confirmPayment() {
        return (
            <TouchableOpacity
                ...
                activeOpacity={0.7}
                onPress={() => {
                    for (let status of this.state.data.accounts) {
                        if (Number(status.available) < Number(this.state.data.amount)) {
...
export default connect(selector)(CartFinalPaymentDetailView);
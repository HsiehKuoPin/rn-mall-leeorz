 ...
 * 跳转并且只保留指定的界面
 */
export const gotoAndClose = createAction(GOTO_AND_CLOSE, (targetRouteName, keepRouteNames, params,duplicateRemoval = true) => ({targetRouteName, keepRouteNames, params,duplicateRemoval}));
 ...
 * @returns {{}}
 */
_gotoAndClose = (state, targetRouteName, keepRouteNames, params,duplicateRemoval) => {
    ...
    tempState.routes.map(item =>{
        if(keepRouteNames.indexOf(item.routeName) > -1){
            if(duplicateRemoval){
                if(pushArr.indexOf(item.routeName) === -1){
                    pushArr.push(item.routeName);
